...
import { createAdmin } from "@/constants/config";
import { useToast } from "@/hooks/use-toast";
import { sighUpAdminSchema } from "@/schema/authSchema";
...
const SignUpAdmin = () => {
  ...
  const form = useForm<z.infer<typeof sighUpAdminSchema>>({
    resolver: zodResolver(sighUpAdminSchema),
    defaultValues: {
      name: "",
      email: "",
      password: "",
  ...
  const onSubmit = async (data: z.infer<typeof sighUpAdminSchema>) => {
    try {
      isLoading(true);
      ...
      <div className="w-full max-w-xl p-8 space-y-8 rounded-xl border border-gray-300 dark:border-gray-700">
        <div className="text-center">
          <p className="text-lg font-semibold tracking-tight lg:text-xl mb-6 text-gray-800 dark:text-gray-200">
            Please fill the following details to sign up as Admin
          </p>
        </div>
        <Form {...form}>
...
const SignUpAdminPage = Layout(SignUpAdmin, {
  title: "Sign Up",
